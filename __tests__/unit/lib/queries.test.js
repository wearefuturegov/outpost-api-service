...
describe("Calling visibleNow", () => {
  it("should return a query that checks if a document is visible now", () => {
    const query = Queries.visibleNow({})
    ...
    expect(query).toEqual({
      $and: [
        ...
        },
        {
          $or: [
            { visible_to: null },
            { visible_to: { $gte: expect.any(Date) } },
          ],
        },
      ],
    })
...
describe("Calling filterAges ", () => {
  ...
  describe("Calling filterOnly", () => {
    it("should return a query that checks if a document is free", () => {
      const req = { query: { only: ["free"] } }
      const query = Queries.filterOnly({}, req)

      // Check if the query contains the correct condition
      expect(query).toEqual({ free: true })
    })
  })
})
